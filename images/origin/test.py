...
def awbImageRead(imageName) :
    ...
    # 위처럼 하려면 roi_boxes를 새로운 변수에 저장 후, while문 돌리기
    ...
    #condition = (roi_boxes.x1 <= 435)
    # print(roi_boxes[condition])
    
    # Make red boxes
    #gray2 = cv.cvtColor(img_result, cv.COLOR_BGR2GRAY)
    #plt.imshow(gray2, cmap='gray')
        
    # 번호판 부분만 추출됐을 때 아래 네 개의 좌표로 crop도 가능함 // 해당 네 개 좌표로 rectangle 처리 또한 가능. 
    #cv.rectangle(img_out, (roi_boxes['x1'].min(), roi_boxes['y1'].min()), (roi_boxes['x2'].max(), roi_boxes['y2'].max()), color=(255, 0, 0), thickness = 2)
    #cv.imwrite(os.path.join(rectangle_path, f'{imageName}.jpg'), img_out)
    #plt.figure(figsize=(12,10))
    #plt.imshow(img_out)
